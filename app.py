...
db_path = "/Users/khangvt/projects/mcp-toolbox-db/db/app.db"
dynamic_sql_handler = DynamicSQLHandler(db_path, client)
    ...
    try:
        ...
        config = types.GenerateContentConfig(
            system_instruction=(
                ...
                '- For **sales volume over time**, use the `sales_by_month` tool. This is for generating charts.\n'
                '- To get a **summary of a customer\'s total spending grouped by order status** (e.g., "how much has customer 29 spent on paid vs. shipped orders?"), you must use the `get_customer_value_by_status` tool.\n'
                '- To get a **detailed list of all orders and the specific items in them** for a customer, use the `get_customer_orders` tool.\n'
                '- To find a customer by name, use `search_customers`.\n'
                '- To view all available products, use `list_products`.\n'
                '- For actions like creating or updating orders, use `create_order`, `add_order_item`, or `update_order_status`.\n\n'
                '- For complex or ad-hoc questions not covered by specific tools, you MUST use `execute_dynamic_sql`. '
                'Pass the user\'s full question as the `natural_language_query` parameter.\n'
                '  - Use for: "Which customers from California spent more than $500?"\n'
                '  - Do NOT use for: "Show me sales by month" (use `sales_by_month` instead).\n'
                'Default Behavior:\n'
        ...
        if fcalls:
            ...
            # --- SPECIAL HANDLING FOR sales_by_month ---
            ...
            # --- GENERIC HANDLING FOR ALL OTHER TOOLS ---
            else:
                tool_result = run_async(mcp_client.invoke_tool(tool_name, args))
...
# (The other endpoints like /api/tools, /debug/mcp, etc., remain unchanged)
@app.route('/api/tools')
def get_tools():
...
def api_charts():
    try:
        ...
        currency = (body.get('currency') or 'VND').strip()
        ...
        try:
            _ = parse_iso(start_date)
            _ = parse_iso(end_date)
        except Exception:
            return jsonify({"error": "Invalid start_date or end_date"}), 400

        if metric != 'sales_by_month':
            return jsonify({"error": "Unsupported metric."}), 400

        args = {"start_date": start_date, "end_date": end_date, "currency": currency}
        tool_result = run_async(mcp_client.invoke_tool("sales_by_month", args))
        
        rows = tool_result.get("results", [])
        data_xy = normalize_sales_by_month(rows, start_date, end_date)
        
        spec = {"kind": kind, "xField": "month_start", "yField": "total_cents", "unit": "month", "currency": currency, "title": "Total sales by month"}
        summary = summarize_series_xy(data_xy)

        return jsonify({"type": "chart", "metric": metric, "spec": spec, "data": data_xy, "summary": summary, "window": {"start_date": start_date, "end_date": end_date}})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

@app.route('/api/charts/sales-by-month', methods=['GET'])
def api_charts_sales_by_month():
    try:
        ...
        try:
            ...
            return jsonify({"error": "Invalid start or end dates"}), 400
        ...
        if out_format == 'csv':
            lines = ["month_start,total_cents"] + [f"{p['x']},{p['y']}" for p in data_xy]
            return Response("\n".join(lines), mimetype="text/csv", headers={"Content-Disposition": "attachment; filename=sales_by_month.csv"})
        
        spec = {"kind": "bar", "xField": "month_start", "yField": "total_cents", "unit": "month", "currency": currency, "title": "Total sales by month"}
        summary = summarize_series_xy(data_xy)
        return jsonify({"type": "chart", "metric": "sales_by_month", "spec": spec, "data": data_xy, "summary": summary, "window": {"start_date": start_date, "end_date": end_date}})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
